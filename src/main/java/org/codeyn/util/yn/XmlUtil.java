...
public class XmlUtil {
    ...
    private static final Document getDocumentFromInputStreamWeblogic(
        ...
        if (!StrUtil.isNull(charset) && "GBK".equalsIgnoreCase(charset)) {// encoding是GBK时，对流进行正确的编码
            // 用缓冲输入流，直接使用输入流时会报:org.xml.sax.SAXParseException: Premature end of
            // file.异常
    ...
    public static final Document createDocument(String rootTagName)
        ...
        return XmlUtil.getDocument(rootTagName);
    }
     ...
     * 输出的xml如果有缩进有回车的话，那么在ff上childNodes数组中也包含回车元素，造成客户端装入xml时还要过滤
    ...
    public static void checkNullNode(Node node) {
        if (node.getNodeType() == Node.TEXT_NODE) {
            ...
            if (nodeValue == null) {
                // throw new RuntimeException("发现文字为null的text节点：" +
                // getNodePath(node));
                throw new RuntimeException(I18N.getString(
                        "com.esen.util.XmlFunc.1", "发现文字为null的text节点：")
                        + getNodePath(node));
            }
        }
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (value == null) {
                    // throw new RuntimeException("发现属性"+attr+"值为null的节点：" +
                    // getNodePath(node));
                    throw new RuntimeException(I18N.getString(
                            "com.esen.util.XmlFunc.2", "发现属性{0}值为null的节点：")
                            + getNodePath(node));
                }
            }
        }
    ...
    public static final String getNodeValue(Node node, String def) {
        ...
        if (node.getNodeType() == Node.CDATA_SECTION_NODE
                || node.getNodeType() == Element.TEXT_NODE) {
            return node.getNodeValue();
        }
        NodeList childNodes = node.getChildNodes();
        for (int i = 0; i < childNodes.getLength(); i++) {
            Node n = childNodes.item(i);
            if (n.getNodeType() == Element.CDATA_SECTION_NODE
                    || n.getNodeType() == Element.TEXT_NODE) {
    ...
    /**
     * 设置node对应的值,如果value=null则转换为value="" 如:value="123" <data>abc</data>
     * 设置后为<data>123</data>
    ...
    public static final void fixTransformerFactory() {
        ...
        try {
            if (System.getProperty("java.version").startsWith("1.5")) {
                 ...
                 * 在tomcat上需要改成com.sun.org.apache.xalan.internal.xsltc.trax.
                 * TransformerFactoryImpl
                 */
                String oldcls = System
                        .getProperty(JAVAX_XML_TRANSFORM_TRANSFORMER_FACTORY);
                try {
                    ...
                    if (System.getProperty("java.vendor").startsWith("IBM")) {
                        System.setProperty(
                                JAVAX_XML_TRANSFORM_TRANSFORMER_FACTORY,
        ...
        if (srcNode.getNodeType() == Node.ELEMENT_NODE) {
            if (srcNode.hasAttributes()) {
                ...
                while (i < len) {
                    ...
                    XmlUtil.setElementAttribute((Element) destNode,
    ...
    public static Element getElementByAttributeInDepth(Document doc,
        ...
        if (StrUtil.isNull(attName) || StrUtil.isNull(attValue)) {
            ...
            throw new RuntimeException(I18N.getString(
                    "com.esen.util.XmlFunc.3", "参数不得为空"));
        }
        Element elem = XmlUtil.getRootElement(doc);
        if (elem == null) {
            return null;
        }
    ...
    public static Element getChildNodeByAttributeInDepth(Element parent,
        ...
        if (StrUtil.isNull(attName) || StrUtil.isNull(attValue)) {
            // throw new RuntimeException("参数不得为空");
            throw new RuntimeException(I18N.getString(
                    "com.esen.util.XmlFunc.4", "参数不得为空 "));
        }
        ...
        while (node != null) {
            if (node.getNodeType() == Node.ELEMENT_NODE) {
                ...
                if (node.hasChildNodes()) {
                    Element elem = getChildNodeByAttributeInDepth(
                            (Element) node, attName, attValue);
                    if (elem != null) {
                        return elem;
                    }
                }
