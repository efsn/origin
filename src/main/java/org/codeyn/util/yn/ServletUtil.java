...
public abstract class ServletUtil {
     ...
     * ，使得该方法在上下两栏时无法正确找到所在的页，所以closeCurrentWindow方法必须指定参数，这里传递的是第二个参数
     * --20110317
     */
    ...
    /**
     * weblogic10.3在windows上好像有个bug，导致bi报错： java.lang.IllegalArgumentException:
     * Bad date header: 'Wed, 03 Jun 2009 11:03:48 GMT' at
     * weblogic.servlet.internal
     * .ServletRequestImpl.getDateHeader(ServletRequestImpl.java:983) at
     * com.esen
     * .irpt.jsp.filter.Filter_js_css.forwardSysjs(Filter_js_css.java:116) at
     * com
     * .esen.irpt.jsp.filter.Filter_js_css.filter_js_css(Filter_js_css.java:61)
     * at com.esen.irpt.jsp.filter.Filter_js_css.doFilter(Filter_js_css.java:45)
     * at
     * weblogic.servlet.internal.FilterChainImpl.doFilter(FilterChainImpl.java
     * :42) Truncated. see log file for complete stacktrace
    ...
    public static void reloadScript(HttpServletRequest req, String object) {
        ...
        js.append("var _java_isWindow_ = ").append(StrUtil.isNull(object))
                .append(";\n");
        js.append("var _java_wnd_ = ")
                .append(StrUtil.null2default(object, "window")).append(";\n");
    ...
    public static String getRemoteAddress(HttpServletRequest req) {
        ...
        if (StrUtil.isNull(ip) || "unknown".equalsIgnoreCase(ip)) {
            ...
            /*
             * 2011-03-28 by
             * hejzh：将"127.0.0.1".equalsIgnoreCase(ip)改为"127.0.0.1"
             * .equals(ip)，实际上字符串
    ...
    public static final String getParameter(HttpServletRequest req, String nm) {
        ...
        if (!StrUtil.isNull(rs)
                && StrUtil.parseBoolean(req.getParameter("escape"), false)) {
     ...
     * 如果url是http或www.开头的话，那么直接sendRedirect 如果是/开头的话也直接sendRedirect
    ...
    /**
     * 设置下载文件的HTTP头 contentType默认为application/x-download charset指定编码,如果没有指定,则不设置
     * contentDisposition默认为attachment;
     * filename=xxx,需要传入文件名,否则使用默认的下载文件名download 如果设置了此参数,则filename无效
    ...
    public static final void setDownloadHeader(HttpServletResponse res,
         ...
         */
        ServletUtil.resetResponse(res);

        if (StrUtil.isNull(contentType))
            contentType = "application/x-download";
        if (StrUtil.isNull(contentDisposition)) {
            if (StrUtil.isNull(filename)) filename = "download";
            ...
            contentDisposition = "attachment; filename="
                    + StrUtil.formatDownloadFileName(filename);
        }
        ...
        /**
         * Content-Disposition参数是为了在客户端另存文件时提供一个建议的文件名,但是考虑到安全的原因，就从规范中去掉了这个参数,
         * 这个不是HTTP/1.1的标准参数
         * 有了Content-Disposition，这样才能使IE使用外部程序打开文件，否则如果是word文件，
         * 那么用户选择“打开”而不是“保存”，那么其实IE是在IE的窗口中打开文件的。
         */
        res.addHeader("Content-Disposition", contentDisposition);
        /**
         * 内容传输编码（Content-Transfer-Encoding），这个区域可以指定ASCII以外的字符编码方式
         * Content-Transfer-Encoding: [mechanism]
         * 其中，mechanism的值可以指定为"7bit"，"8bit"
         * ，"binary"，"quoted-printable"，"base64"。 7bit:7字节的ASCII编码方式。
         * 8bit:8比特ASCII码. quoted-printable:因为欧洲的一些文字和ASCII字符集中的某些字符有部分相同。
         * 如果邮件消息使用的是这些语言的话，于ASCII重叠的那些字符可以原样使用，ASCII字符集中不存在的字符采用形如“=??”的方法编码。
         * 这里“??”需要用将字符编码后的16进制数字来指定。采用quoted-printable编码的消息，长度不会变得太长，
         * 而且大部分都是ASCII中的字符， 即使不通过解码也大致可以读懂消息的内容。
    ...
    /**
     * 执行HttpServletResponse的reset方法，其实只调用res.reset()即可，但是websphere上有bug，
     * 调用reset并不能清空header， 所以此处用这个方法封装了一下。
     * websphere的bug参考：http://www-01.ibm.com/
     * support/docview.wss?rs=180&uid=swg1PK29451 PK29451: A CALL TO
    ...
    public static String getRequestURI_withoutContextPath(
        ...
        if (r != null && r.indexOf("//") > -1) {
            /**
             * BIDEV-827
             * 在IE中访问这个地址：http://192.168.1.200:8080/bi2.1//editrpt.do?action
             * =edit&taskid=LKU5BCLYNUU6S5MKLVWCC8LUJ1KTUXS2&rptid=
             * LKU5BCLYNUU6S5MKLVWCC8LUJ1KTUXS2$1$ZYL4K39I8PL1I4LYCTBUU97KMIYDMTLL
             * 浏览器总是报错。特点是bi2.1后面有2个斜线。 所以这里处理了一下，把//换成/
             */
            r = r.replaceAll("//+", "/");
        }
    ...
    public static String getContextPath(HttpServletRequest httpRequest) {
        ...
        if (ctxpath.charAt(ctxpath.length() - 1) != '/') ctxpath += '/';

        return ctxpath;
    }

    /**
     * HttpServletRequest的getParameterMap方法获取的map中value都是字符串数组对象，
    ...
    public static String encodeURL(String url, String enc)
         ...
         * 这是jdk的BUG
         * ，URLEncoding会把" "encode为"+"，导致url被decode时，"+"没有被转成" "，仍然是"+"。
    ...
    public static String forwardPage(final HttpServletRequest req,
         ...
         * 参考页面：https
         * ://issues.apache.org/jira/browse/STR-3156?page=com.atlassian
         * .jira.plugin.system.issuetabpanels%3Achangehistory-tabpanel
         */
    ...
    public static void includeJspResult(ServletRequest req,
        ...
        out.print(src);
        out.print("] -->");
        req.getRequestDispatcher(src).include(
                req,
                new HttpServletResponseWrapper4includeJspResult(
    ...
    /**
     * 根据语言代码返回期望的Locale对象。不支持时，返回Locale.ENGLISH。 匹配语言代码的顺序是：
        ...
        if (StrUtil.isNull(langStr)) {
            ...
            if (cookies != null) {
                for (int i = 0; i < cookies.length; i++) {
                    if ("esensoft.user.language".equalsIgnoreCase(cookies[i]
                            .getName())) {
            ...
            if (StrUtil.isNull(langStr)) langStr = req.getLocale().toString();
        }
...
/**
 * 该类扩展了HttpServletResponseWrapper类，主要是为了在自定义标签实现中通过include方法来引入的jsp或者html时，
 * 能够正确的显示在指定区域内
 */
