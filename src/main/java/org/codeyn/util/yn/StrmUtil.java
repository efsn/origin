...
public final class StrmUtil {
    ...
    public static Reader toReader(String filePath, String charset)
        ...
        Reader in = null;
        if (charset != null) {
            // chinese in GBK more than GB2312
             ...
             * 导入SybaseASE，出现Error converting characters into server's character
             * set. Some character(s) could not be converted. 的异常； 解决：
             * 通过UnicodeReader来读取，过滤BOM标识；
             */
            in = new UnicodeReader(new FileInputStream(f), charset);
        } else {
            /*
             * ESENFACE-1079: add by liujin 2014.08.28 因为该 bug 的修改，导出的 UTF-8 编码的
             * CSV 文件中添加了文件头 '\ufeff' 以便 excel、记事本等工具识别文件编码。
    ...
    static public final String readString(InputStream in, int bufsize,
        ...
        return StrUtil.isNull(charset) ? new String(buf, 0, i) : new String(buf,
                0, i, charset);
    }
    ...
    public static final String readLinesUntil(InputStream br, String ln,
        ...
        while ((s != null) && !s.equals(ln)) {
            ...
            if ((s != null) && !s.equals(ln)) result.append("\r\n");
        }
    ...
    public static final byte[] stm2bytes(InputStream in) throws IOException {
        ...
        return out.getBuf().length == out.size() ? out.getBuf() : out
                .toByteArray();
    }
    ...
    static public final byte[] gzipStm(InputStream in) throws IOException {
        ...
        GZIPOutputStream gout = new GZIPOutputStream(out);
        try {
            StrmUtil.stmTryCopyFrom(in, gout);
        } finally {
            gout.close();
        }
        return out.toByteArray();
    ...
    public static final InputStream getUnGZIPStm(InputStream in)
        ...
        if ((ch2 << 8) + (ch1 << 0) == GZIP_MAGIC) {
             ...
             * ByteArrayInputStream(StmFunc.gzipBytes("abc123".getBytes()));
             * InputStream in = new GZIPInputStream(new
             * BufferedInputStream(bin)); if (!in.markSupported()) { in = new
    ...
    static public final void writeInt(final Writer writer, int i)
        ...
        for (int j = StrUtil.TEN_POWERS.length - 1; j >= 0; j--) {
            int v = i / StrUtil.TEN_POWERS[j];
            if (v > 0) {
    ...
    public static int escape(final CharSequence s, Writer out)
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (StrUtil.needEscape(ch)) {
                ...
                out.write('%');
                out.write(StrUtil.HEX_0_FF[ch]);
                r += 3;
            } else { // unicode : map to %uXXXX
                out.write('%');
                out.write('u');
                out.write(StrUtil.HEX_0_FF[(ch >>> 8)]);
                out.write(StrUtil.HEX_0_FF[(0x00FF & ch)]);
                r += 5;
            }
        }
        return r;
