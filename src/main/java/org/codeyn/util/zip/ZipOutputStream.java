...
public
  ...
  /**
   * 往压缩流里面写一个文件，可以是文件，也可以是目录，如果是目录，进行递归压缩。
   * @param srcFile      待压缩的文件
   * @param name         压缩后的名称，如果是目录，是相对目录名
   * @param ignoreEmptyDir 如果是压缩目录，是否忽略空目录
   */
  public void writeFile(File srcFile, String name,  boolean ignoreEmptyDir) throws Exception {
	  if(srcFile.isDirectory()){
		  File[] fl = srcFile.listFiles();
	      if (!ignoreEmptyDir) {
				/**
				 * 在生成zip文件时，如果name=a,在windows下name + File.separator=a\\,写入到zip中时会生成一个文件名为空的文件
				 * 所以这里改成name + "/"
				 */
	      ...
	      for (int i = 0; i < fl.length; i++) {
	    	  writeFile(fl[i], name + fl[i].getName(), ignoreEmptyDir);
	      }
	  }else{
