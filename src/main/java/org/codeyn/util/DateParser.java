...
//    Pattern p = Pattern.compile("(\\d{4})([\u4e00-\u9fa5]|[\\-/,，\\*\\\\]{1,}|n|)(\\d{2}|\\d{1})?([\u4e00-\u9fa5]|[\\-/,，\\*\\\\]{1,}|y|)(\\d{2}|\\d{1})?([\u4e00-\u9fa5]|[\\-/,，\\*\\\\]{1,}|r|)",
//                                Pattern.CASE_INSENSITIVE);
    Pattern p = Pattern.compile("(\\d{4})([^\\d]{0,})(\\d{2}|\\d{1})?([^\\d]{0,})(\\d{2}|\\d{1})?([^\\d]{0,})");
    ...
    if(m.find()) {  
      ...
      //throw new IllegalArgumentException("不是正确的时间格式."); //by xh
    	//throw new IllegalArgumentException(I18N.getString("com.esen.util.DateParser.2", "不是正确的时间格式."));
    }
  }
  
  private int parseYear(String year) {
    return (year == null || year.length() == 0 ? Calendar.getInstance().YEAR : Integer.parseInt(year));
  }
  
  private int parseMonth(String month) {
    int _month = (month == null || month.length() == 0 ? -1 : Integer.parseInt(month));
    return _month >= 1 && _month <= 12 ? _month : -1;
  }
  
  private int parseDay(String day) {
    //TODO 考虑各个月份及闰年的情况.
    int _day = (day == null || day.length() == 0 ? -1 : Integer.parseInt(day));
