...
public class TxtLoader {
  ...
  public void prepareRead(byte[] bytes, int size, String charsetName) throws UnsupportedEncodingException {
    prepareRead(new MyByteArrayInputStream(bytes, 0, size), charsetName); 
  }

  public void prepareRead(InputStream stm) throws UnsupportedEncodingException  {
    prepareRead(stm, null);
  }

  public void prepareRead(InputStream stm, String charsetName) throws UnsupportedEncodingException  {
    this.in = new BufferedReader2(StrYn.isNull(charsetName)?new InputStreamReader(stm):new InputStreamReader(stm, charsetName));
  }
...
//    return new String(r, 0, count);
  ...
  /**
   * 读取所有行内容直到碰到与tagName匹配的结束标签
   * 会移动读流指针位置，且将结束标签行也读完
   ...
   */
  public final String readTagContent(String tagName, boolean recursive) throws IOException{
  ...
  public final String getTagName(final String s) {
    ...
    if (i >= 0)
      return s.substring(1, i);
    	...
    	//使用readtag读出的字符串 形式有 <aaaa>   <aaaa   <aaaa/> ,所以以下代码有问题 update by jzp 2011-11-08
  ...
  public final boolean isTag(final String s, final String tag) {
    ...
    return (s != null) && s.startsWith(tag) && (s.length() > tag.length())
   ...
   */
  public final StringMap getContentMap(final String tagEnd) throws IOException {
	  /**
	   * BUG: 20110824
   ...
   * @throws IOException
   */
  public final StringMap getContentMap2(final String tagEnd) throws IOException {
    return (StringMap) in.readMap(tagEnd, '\n', '=', new StringMap());
  }
  
  public final Map getContentMap(final String tagEnd, Map map) throws IOException {
    return (Map) in.readMap(tagEnd, '\n', '=', map);
  }
  ...
  /**
   * 为了内存效率 tagAttr 共用一个对象，所以此对象不能给外部直接持有此对象
   * 必须拷贝一份使用
   * @return
   */
  ...
  public StringMap getTagMap(final String line){
    ...
    if (i > 0) {
      String attr = line
          .substring(i + 1, line.length() - (isTagEnd(line) ? 2 : 1));
   ...
   * @throws Exception
   */
  public static void throwUnidentificationException(String tag) throws Exception{
