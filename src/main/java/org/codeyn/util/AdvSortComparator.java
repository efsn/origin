 ...
 * 支持中文按拼音排序
 * 支持null的处理
 * 升序时NULL排序到最后  isAscNull 默认为false即升序时NULL排序到最前
 * 降序时NULL排序到最前  isDescNull  默认为false即降序时NULL排序到最后
 */
...
public class AdvSortComparator implements Comparator {
  ...
  public int compare(Object a, Object b) {
    ...
    if (nulla && nullb)
      return 0;
    if (nulla && !nullb) {
      if (asc && isAscNullLast) 
        return 1;
      if (!asc && isDescNullFirst) 
        return -1;
      return asc?-1:1;
    }
    if (!nulla && nullb) {
  ...
  private Collator getLocalStrCompare() {
    if (this.localstrcmp == null)
      this.localstrcmp = java.text.Collator.getInstance(java.util.Locale.CHINA);//try testing various locales
  ...
  private boolean isNull(Comparable o) {
    ...
    if (o instanceof Double)
      ...
      /**
       * 20080730 比较时，空串和null等价。
       * 20080801 上一次的改动总是认为把空串和null等价，这改变了本类的接口，可能会影响其它程序。
       *          增加isNullEqualBlank参数，它默认是false；设置为true时认为空串和null等价。
       */
